...
public class Match implements Runnable {
    ...
    private final List<Team> squadList; // Premade teams
    ...
    private final int[] pickSequence;
...
//		}
        ...
        if (sortedPlayers.size() > 2) {
            ...
            for (Player p : sortedPlayers) {
                ...
                if (logic.getDynamicServers()) {
                    captainAnnouncement += " " + "\t " + server.playerPing.get(p) + "ms";
                }
            }
            if (logic.getDynamicServers()) {
    ...
    public void run() {
        ...
        button.custom_id = Config.INT_LAUNCHAC + "_" + id + "_" + server.getAddress() + "_" + server.password;
    ...
    public DiscordEmbed getMatchEmbed(boolean forceNoDynamic) {
        ...
        for (Map.Entry<Player, MatchStats> entry : entries) {
            ...
            if ((logic.getDynamicServers() || gametype.getTeamSize() == 0) && !forceNoDynamic) {
                ping_row = server.playerPing.get(entry.getKey()) + "\n";
            }
