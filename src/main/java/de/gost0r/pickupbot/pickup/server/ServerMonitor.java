...
public class ServerMonitor implements Runnable {
    ...
    private final Map<String, CTF_Stats> backupStats;
    ...
    public void run() {
        ...
        try {
            ...
            Sentry.capture(e);
            match.getLogic().bot.sendMsg(match.getLogic().getChannelByType(PickupChannelType.ADMIN), "ServerMonitor " + e);
        }
    ...
    private void checkNoMercy(RconPlayersParsed rpp) {
        if (Math.abs(rpp.scores[0] - rpp.scores[1]) >= 10 && !noMercyIssued && match.getGametype().getTeamSize() > 2) {
            ...
            sendDiscordMsg("**[MERCY RULE]** The match #" + match.getID() + " is a massacre and will end in 1min.");
        }
    }
    ...
    private void updatePlayers(RconPlayersParsed rpp) throws Exception {
        ...
        for (ServerPlayer player : rpp.players) {
            ...
            if (player.state == ServerPlayerState.Connecting || player.name.equals("GTV-b00bs"))
...
//		LOGGER.fine("rcon players: >>>" + playersString + "<<<");
        ...
        for (String line : stripped) {
            ...
            if (line.startsWith("Map:")) {
                ...
                rpp.matchmode = line.split(" ")[1].equals("ON");
            } else if (line.startsWith("MatchReady")) {
                rpp.matchready[0] = line.split(" ")[1].split(":")[1].equals("YES");
                rpp.matchready[1] = line.split(" ")[2].split(":")[1].equals("YES");
            } else if (line.startsWith("WarmupPhase")) {
                rpp.warmupphase = line.split(" ")[1].equals("YES");
...
//				LOGGER.severe("splitted = " + Arrays.toString(splitted));
                ...
                if (splitted[0].equals("CTF:") && awaitsStats) {
                    // ctfstats
                    rpp.players.get(rpp.players.size() - 1).ctfstats.caps = Integer.parseInt(splitted[1].split(":")[1]);
                    rpp.players.get(rpp.players.size() - 1).ctfstats.returns = Integer.parseInt(splitted[2].split(":")[1]);
                    rpp.players.get(rpp.players.size() - 1).ctfstats.fc_kills = Integer.parseInt(splitted[3].split(":")[1]);
                    rpp.players.get(rpp.players.size() - 1).ctfstats.stop_caps = Integer.parseInt(splitted[4].split(":")[1]);
                    rpp.players.get(rpp.players.size() - 1).ctfstats.protect_flag = Integer.parseInt(splitted[5].split(":")[1]);
                    awaitsStats = false;
